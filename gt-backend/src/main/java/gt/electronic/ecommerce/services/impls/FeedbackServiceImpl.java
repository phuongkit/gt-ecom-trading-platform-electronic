...
public class FeedbackServiceImpl implements FeedbackService {
  ...
  @Autowired public void CommentRepository(CommentRepository commentRepo) {
    this.commentRepo = commentRepo;
  }

  private FeedbackMapper feedbackMapper;

  @Autowired public void FeedbackMapper(FeedbackMapper feedbackMapper) {
  ...
  @Override public Page<FeedbackResponseDTO> getAllMainFeedbacksByProduct(
  ...
  @Override public Page<FeedbackResponseDTO> getAllRelyFeedbacksByMainFeedback(
      Long mainFeedbackId,
      Pageable pageable
  ) {
  ...
  @Override public FeedbackResponseDTO getFeedbackById(Long id, boolean isHasChild) {
  ...
  @Override public FeedbackResponseDTO getFeedbackByProductAndUser(Long productId, Integer userId, boolean isHasChild) {
    Product productFound = this.productRepo.findById(productId).orElseThrow(() -> new ResourceNotFound(
        String.format(Utils.OBJECT_NOT_FOUND_BY_FIELD, Product.class.getSimpleName(), "ID", productId)));
    User userFound = this.userRepo.findById(userId).orElseThrow(() -> new ResourceNotFound(
        String.format(Utils.OBJECT_NOT_FOUND_BY_FIELD, User.class.getSimpleName(), "ID", userId)));
  ...
  @Override public ProductRating getProductRatingByProduct(Long productId) {
  ...
  @Override public FeedbackResponseDTO createFeedback(
    ...
    if (creationDTO.getReplyForFeedbackId() != null) {
      mainFeedbackFound = this.feedbackRepo.findById(creationDTO.getReplyForFeedbackId()).orElseThrow(() ->
                                                                                                          new ResourceNotFoundException(
                                                                                                              String.format(
                                                                                                                  Utils.OBJECT_NOT_FOUND_BY_FIELD,
                                                                                                                  "Main" + branchName,
      ...
      if (feedbacks.size() > 0) {
        throw new ResourceAlreadyExistsException(
            String.format(Utils.OBJECT_EXISTED_BY_TWO_FIELD, branchName,
                          Product.class.getSimpleName(), creationDTO.getProductId(), User.class.getSimpleName(),
      ...
      if (imageGalleryFile != null && imageGalleryFile.length > 0) {
        ...
        for (MultipartFile multipartFile : imageGalleryFile) {
          if (!multipartFile.isEmpty()) {
            Image image = this.imageService.createImageByMultipartFile(multipartFile, EImageType.IMAGE_FEEDBACK);
  ...
  @Override public FeedbackResponseDTO updateFeedback(
  ...
  @Override public FeedbackResponseDTO deleteFeedbackById(Long id) {
