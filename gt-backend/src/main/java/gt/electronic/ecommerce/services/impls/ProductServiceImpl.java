...
public class ProductServiceImpl implements ProductService {
    ...
    public Page<ProductGalleryDTO> filterProduct(
        ...
        if (categoryName != null) {
            ...
            categoryFound =
                    this.categoryRepo.findByName(categoryName).orElseThrow(() -> new ResourceNotFound(String.format(
    ...
    public Page<ProductGalleryDTO> filterProductByKeyword(
        ...
        Page<Product> productPage =
                this.productRepo.filterProductToPage(keyword, null, null, null, locationFound, minPrice, maxPrice,
                                                     pageable);
        if (productPage.getContent().size() < 1) {
            throw new ResourceNotFound(String.format(Utils.OBJECT_NOT_FOUND, branchName));
        }
...
//    boolean hasBrand = false, hasCategory = false;
        ...
        if (sortOption == ESortOption.POPULAR.ordinal() || sortOption == ESortOption.TOP_SALES.ordinal() || order != null) {
            ...
            if (sortOption == ESortOption.TOP_SALES.ordinal()) {
                ...
                productList.sort((o1, o2) -> {
                    ...
                    double delta =
                            rating1.getStar() * rating1.getTotalVote() - rating2.getStar() * rating2.getTotalVote();
                    if (delta == 0) {
                        if (order != null) {
                            return comparePriceProduct(o1, o2, order);
                        }
    ...
    public ProductResponseDTO updateProduct(
        ...
        Product entityFound =
                this.productRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException(String.format(
                        Utils.OBJECT_NOT_FOUND_BY_FIELD,
                        branchName,
                        "ID",
                        id)));
