...
//                                   "LoginKey",
  ...
  @Override public List<DiscountResponseDTO> addAllDiscountToUser(String loginKey, DiscountCodes discountCodes) {
    ...
    for (String discountCode : discountCodes.getDiscountCodes()) {
      ...
      if (!userFound.getDiscounts().contains(discountFound)) {
        ...
        throw new ResourceAlreadyExistsException(String.format(Utils.OBJECT_EXISTED_BY_FIELD, User.class.getSimpleName(), "discountCode", discountCode));
      }
    }
    this.userRepo.save(userFound);
    return discounts.stream().map(discount -> this.discountMapper.discountToDiscountResponseDTO(discount)).collect(
  ...
  @Override public List<DiscountResponseDTO> removeAllDiscountFromUser(String loginKey, DiscountCodes discountCodes) {
