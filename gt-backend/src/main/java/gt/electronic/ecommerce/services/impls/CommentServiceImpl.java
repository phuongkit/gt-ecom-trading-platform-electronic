...
public class CommentServiceImpl implements CommentService {
  ...
  public Page<CommentResponseDTO> getAllCommentsByUser(
    ...
    Page<Comment> comments =
        this.commentRepo.getAllCommentByProductAndUser(null, userFound, pageable);
    if (comments.getContent().size() < 1) {
      throw new ResourceNotFound(
          String.format(Utils.OBJECT_NOT_FOUND, branchName));
    }
                ...
                () ->
                    new ResourceNotFound(
                        String.format(
                            ...
                            userId)));
    Page<Comment> comments =
        this.commentRepo.getAllCommentByProductAndUser(productFound, userFound, pageable);
    if (comments.getContent().size() < 1) {
      throw new ResourceNotFound(
          String.format(Utils.OBJECT_NOT_FOUND, branchName));
    }
                ...
                () ->
                    new ResourceNotFound(
                        String.format(
                            ...
                            mainCommentId)));
    Page<Comment> comments =
        this.commentRepo.getAllChildCommentByMainComment(mainCommentFound, pageable);
    if (comments.getContent().size() < 1) {
      throw new ResourceNotFound(
          String.format(Utils.OBJECT_NOT_FOUND, branchName));
    }
  ...
  @Override public CommentResponseDTO createComment(String loginKey, CommentCreationDTO creationDTO,
    ...
    if (imageGalleryFile != null && imageGalleryFile.length > 0) {
      ...
      for (MultipartFile multipartFile : imageGalleryFile) {
        if (!multipartFile.isEmpty()) {
          Image image =
              this.imageService.createImageByMultipartFile(multipartFile, EImageType.IMAGE_COMMENT);
...
//      Long relyCommentId, RelyCommentCreationDTO creationDTO, MultipartFile[] imageGalleryFile) {
//    this.LOGGER.info(
//        String.format(
//            Utils.LOG_CREATE_OBJECT_BY_TWO_FIELD,
//            "Rely" + branchName,
