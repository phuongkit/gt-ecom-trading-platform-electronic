...
public class CartItemServiceImpl implements CartItemService {
  ...
  @Autowired public void SaleService(SaleService saleService) {
  ...
  public List<OrderDetailResponseDTO> getAllCartItemByUser(String loginKey, Long userId, Pageable pageable) {
    ...
    List<OrderItem> cartItemList = this.orderItemRepo.findByUserAndStatus(userFound, EOrdertemStatus.UN_PAID, pageable);
    return cartItemList.stream().map(orderItem -> this.orderItemMapper.orderItemToOrderDetailResponseDTO(orderItem))
        .collect(
            Collectors.toList());
  }
                ...
                () ->
                    new ResourceNotFoundException(
                        String.format(
                            ...
                            userId, Product.class.getSimpleName(), productId)));
    return this.orderItemMapper.orderItemToOrderDetailResponseDTO(cartItemFound);
  }
  ...
  public OrderDetailResponseDTO createCartItem(String loginKey, CartItemCreationDTO creationDTO) {
    ...
    if (cartItemFound == null) {
      ...
      cartItemFound.setQuantity(creationDTO.getQuantity() < productFound.getQuantity() ? cartItemFound.getQuantity() :
                                    productFound.getQuantity());
      cartItemFound.setSale(this.saleService.getMostOptimalSaleByProduct(productFound.getId()));
      cartItemFound.setTotalPrice(Utils.getPriceProduct(cartItemFound.getProduct(), cartItemFound.getSale()));
  ...
  @Override public List<OrderDetailResponseDTO> updateCart(
      String loginKey,
      List<CartDetailCreationDTO> creationDTOList
  ) {
    this.LOGGER.info(
        String.format(Utils.LOG_UPDATE_OBJECT + Utils.ADD_LOG_FOR_USER, "List cart", "Length", creationDTOList.size(),
                      "loginKey", loginKey));
    User userFound;
    if (Utils.isPattern(loginKey) == EPattern.PHONE) {
      userFound = this.userRepo
    ...
    this.orderItemRepo.saveAll(this.orderItemMapper.cartDetailCreationDTOsToOrderItems(userFound, creationDTOList));
    orderItemList = this.orderItemRepo.findByUserAndStatus(userFound, EOrdertemStatus.UN_PAID, null);
    return orderItemList.stream().map(orderItem -> this.orderItemMapper.orderItemToOrderDetailResponseDTO(orderItem))
        .collect(
            Collectors.toList());
  }
}
