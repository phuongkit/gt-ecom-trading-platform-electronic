...
public class OrderShopServiceImpl implements OrderShopService {
  ...
  @Override public Page<OrderResponseDTO> getAllOrderShops(Pageable pageable) {
  ...
  @Override public Page<OrderResponseDTO> getAllOrderShopsByUser(String loginKey, Integer userId, Pageable pageable) {
  ...
  @Override public Page<OrderResponseDTO> getAllOrderShopsByShop(
      String loginKey,
      Integer shopId,
      Pageable pageable,
      boolean... isAdmin
  ...
  @Override public OrderResponseDTO getOrderShopByOrderId(String loginKey, Long orderId) {
    this.LOGGER.info(String.format(Utils.LOG_GET_OBJECT + Utils.ADD_LOG_FOR_USER,
                                   branchName,
                                   "ID",
                                   orderId,
  ...
  public OrderResponseDTO updateStatusOrderShop(String loginKey, Long id, OrderUpdateStatusDTO updateStatusDTO) {
    ...
    OrderShop entityFound = this.orderShopRepo.findById(id)
        .orElseThrow(() -> new ResourceNotFoundException(String.format(Utils.OBJECT_NOT_FOUND_BY_FIELD,
                                                                       ...
                                                                       "ID",
                                                                       id)));
    if (entityFound.getOrder() != null && entityFound.getOrder().getUser() == null || Objects.equals(userFound.getId(),
                                                                                                     entityFound.getOrder()
                                                                                                         .getUser()
                                                                                                         .getId()) || userFound.getRole()
