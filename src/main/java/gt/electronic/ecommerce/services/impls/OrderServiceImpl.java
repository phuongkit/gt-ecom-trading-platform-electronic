...
//    }
      ...
      for (Long discountId : creationDTO.getDiscountIds()) {
        ...
        try {
          if (discountFound != null && Utils.checkValidDiscount(discountFound)) {
            OrderShop orderShop = Utils.getOrderShopFromList(orderShops, discountFound.getShop(), savedEntity);
    ...
    BigDecimal[] total = new BigDecimal[]{new BigDecimal(0), new BigDecimal(0), new BigDecimal(0), new BigDecimal(0)};
    ...
    for (OrderShop orderShop : orderShops) {
      ...
      for (Discount discount : orderShop.getDiscounts()) {
        if (discount.getType() == DISCOUNT_SHOP_PRICE) {
          ...
          BigDecimal finalPriceDiscount =
              orderShop.getTotalPriceProduct().multiply(BigDecimal.valueOf(discount.getPercent()));
      ...
      for (OrderShopCreationDTO orderShopCreationDTO : creationDTO.getOrderShops()) {
        if (Objects.equals(orderShopCreationDTO.getShopId(), orderShop.getShop().getId())) {
          orderShop.setShippingMethod(orderShopCreationDTO.getShippingMethod() == null ? EShippingMethod.GHN_EXPRESS :
