...
@Service @Transactional public class ProductServiceImpl implements ProductService {
  ...
  @Autowired public void BrandRepository(BrandRepository brandRepo) {
    this.brandRepo = brandRepo;
  }

  private CategoryRepository categoryRepo;

  @Autowired public void CategoryRepository(CategoryRepository categoryRepo) {
    this.categoryRepo = categoryRepo;
  }

  private DescriptionService descriptionService;

  @Autowired public void DescriptionService(DescriptionService descriptionService) {
  ...
  @Autowired public void ProductDescriptionService(ProductDescriptionService productDescriptionService) {
  ...
  @Override public Page<ProductGalleryDTO> getAllProduct(Pageable pageable) {
  ...
  @Override public Page<ProductGalleryDTO> getAllProductOnTradingByPage(Pageable pageable) {
  ...
  @Override public Page<ProductGalleryDTO> filterProduct(
      String keyword,
      String brandName,
      String categoryName,
      String shopName,
  ...
  @Override public Page<ProductGalleryDTO> filterProductByKeyword(
      String keyword, String locationString, int sortOption, BigDecimal minPrice, BigDecimal maxPrice, Pageable pageable
  ) {
    this.LOGGER.info(String.format(Utils.LOG_GET_ALL_OBJECT_BY_FIELD, branchName, "Keyword", keyword));
    Location locationFound = null;
    if (locationString != null && locationString.length() > 0) {
    ...
    Page<Product> productPage =
        this.productRepo.filterProductToPage(keyword, null, null, null, locationFound, minPrice, maxPrice, pageable);
    if (productPage.getContent().size() < 1) {
      throw new ResourceNotFound(String.format(Utils.OBJECT_NOT_FOUND, branchName));
    }
    return productPage.map(product -> this.productMapper.productToProductGalleryDTO(product));
...
//    boolean hasBrand = false, hasCategory = false;
  ...
  @Override public ProductResponseDTO createProduct(
      String loginKey,
    ...
    if (thumbnailFile != null && !thumbnailFile.isEmpty()) {
      Image thumbnailImage = this.imageService.createImageByMultipartFile(thumbnailFile, EImageType.IMAGE_PRODUCT);
    ...
    if (imageGalleryFile != null && imageGalleryFile.length > 0) {
      ...
      for (MultipartFile multipartFile : imageGalleryFile) {
        if (!multipartFile.isEmpty()) {
          Image image = this.imageService.createImageByMultipartFile(multipartFile, EImageType.IMAGE_PRODUCT_GALLERY);
    ...
    if (creationDTO.getDescriptions() != null) {
      ...
      for (var entry : creationDTO.getDescriptions().entrySet()) {
        ...
        saved.addDescription(this.productDescriptionService.createOrUpdateProductDescription(productDescription));
      }
    }
  ...
  @Override public ProductResponseDTO updateProduct(
      String loginKey,
      Long id,
      ProductCreationDTO creationDTO,
      MultipartFile thumbnailFile,
    ...
    Product entityFound = this.productRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException(String.format(
        Utils.OBJECT_NOT_FOUND_BY_FIELD,
        branchName,
        "ID",
        id)));
  ...
  @Override public ProductResponseDTO deleteProductById(String loginKey, Long id, boolean... isAdmin) {
  ...
  @Override public void createDescription(Product product, Map<String, String> descriptions) {
    ...
    if (descriptions != null) {
      ...
      for (var entry : descriptions.entrySet()) {
        ...
        //        productDescription.setValue(descriptionDTO.getValue());
        productDescription.setValue(entry.getValue());
        product.addDescription(this.productDescriptionService.createOrUpdateProductDescription(productDescription));
      }
    }
  }
